...
###
Bots.registerComponent 'bot',
  publish: -> { @name, @owner, @plan }

  created: (ent, params) ->
    @name = 'Robot ' + Random.id().slice(0, 4)
    @owner = params.owner
    @plan = []
  ...
  advance: (ent, delta) ->
      ...
      switch action.type
        when 'walk'
          ...
          if dist > 0
            ...
            angleDiff = ang - transform.rot
            stepSize = 5 * Math.max -0.02, (Math.cos angleDiff) - 0.8
            stepSize = Math.max stepSize, dist - 3
            stepSize = delta * Math.min 1, stepSize
            if dist > stepSize
              fwdX = stepSize * Math.cos transform.rot
              fwdY = stepSize * Math.sin transform.rot
              # if factor < 1 then dx *= factor; dy *= factor
              pos[0] += fwdX
              pos[1] += fwdY
            else
    ...
    #       @plan.push
    #         type: 'walk'
    #         pos: [ Math.random() * 10 - 5, Math.random() * 10 - 5, 0 ]
    return
